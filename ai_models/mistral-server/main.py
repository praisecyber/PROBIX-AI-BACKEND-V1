...
app = FastAPI(title="Mistral Local Server (Hybrid)")
...
MODEL_PATH = os.getenv("MISTRAL_MODEL_PATH", os.path.join(project_root, "ai_models", "models"))
MODEL_FILENAME = os.getenv("MISTRAL_MODEL_FILENAME", "mistral-7b-instruct-v0.2.Q4_K_M.gguf")
...
OPENAI_MODEL = os.getenv("MISTRAL_MODEL", "gemini-2.0-flash")
    ...
    try:
        ...
        if llama:
            try:
                ...
                for msg in messages:
                    if msg["role"] == "user":
                        prompt_text += f"<s>[INST] {msg['content']} [/INST]"
        ...
        if "hi" in prompt_lower or "hello" in prompt_lower or "hey" in prompt_lower:
            return GenerateResponse(response="Hi there! I'm Mistral! How can I help you today?")
        elif "who are you" in prompt_lower:
            return GenerateResponse(response="I'm Mistral, your AI assistant! I can help with coding, explanations, and more!")
        else:
